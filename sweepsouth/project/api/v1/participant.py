...
@participant.route('/participant/extension/all')
class participantext(Resource):
    @token_required
    def get(self):
        if request.args:
            start = request.args.get('start', None)
            ...
            count = request.args.get('count', None)
            # Still to fix the next and previous WRT Sqlalchemy
            next = "/api/v1/participant/extension/all?start=" + \
                str(int(start)+1)+"&limit="+limit+"&count="+count
            previous = "/api/v1/participant/extension/all?start=" + \
                str(int(start)-1)+"&limit="+limit+"&count="+count
            
            URL="http://195.15.218.172/participant/extension/tous"
            r = requests.get(url=URL)
            if r.status_code == 200:
                return {
                    "start": start,
...
@participant.route('/participant/voie/all')
class participantvoie(Resource):
    @token_required
    def get(self):
        if request.args:
            start = request.args.get('start', None)
            ...
            count = request.args.get('count', None)
            # Still to fix the next and previous WRT Sqlalchemy
            next = "/api/v1/participant/voie/all?start=" + \
                str(int(start)+1)+"&limit="+limit+"&count="+count
            previous = "/api/v1/participant/voie/all?start=" + \
                str(int(start)-1)+"&limit="+limit+"&count="+count
            
            URL="http://195.15.218.172/participant/voie/tous"
            r = requests.get(url=URL)
            if r.status_code == 200:
                return {
                    "start": start,
