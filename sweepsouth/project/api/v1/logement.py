...
@logement.route('/logement/logement/by_ref/')
class logement_byref(Resource):
    def get(self):
        if request.args:
            start = request.args.get('ref', None)
            URL="http://195.15.228.250/logement/logement/by_ref/"+start
            r = requests.get(url=URL)
            if r.status_code == 200:
                return {
                ...
                return{
                    "res":"logement by ref service down"
                }, 400
#client route
@logement.doc(
    security='KEY',
